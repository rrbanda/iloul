"""
Workflow management classes for mortgage application processing
Provides high-level interfaces for managing LangGraph workflows
"""
...
class MortgageApplicationAgent:
    ...
    def __init__(self, use_persistent_storage: bool = True, db_path: str = "data/mortgage_applications.db"):
        ...
        # TODO: In production, replace with PostgresStore or similar when available
        # self.store = InMemoryStore()  # Disabled for now - not available in current LangGraph version
        self.store = None
        print("💾 Memory store disabled (using checkpointer only)")
        
        # The create_mortgage_graph() function returns an already compiled graph
        self.graph = create_mortgage_graph()
        # Note: checkpointer and store configuration might need to be handled differently
    ...
    def chat(self, message: str, thread_id: str = "default", user_id: str = None, 
        ...
        # Note: durability parameter not supported in current LangGraph version
        ...
        try:
            ...
            # Intelligent application lifecycle integration
            self._handle_application_lifecycle_integration(
                thread_id, user_id, message, result, context
            )
    ...
    def _handle_application_lifecycle_integration(self, 
                                                thread_id: str, 
                                                user_id: str, 
                                                message: str, 
                                                result: Dict[str, Any], 
                                                context: MortgageContext) -> None:
        """
        Intelligently integrate conversation with unified application lifecycle.
        
        This method detects when applications should be created or linked based on 
        conversation content and user intent, preventing conflicts between chat and 
        document upload workflows.
        """
        try:
            app_manager = get_application_manager()
            
            # Extract conversation context from result messages
            conversation_context = self._extract_conversation_context(result, context)
            
            # Detect user intent from current message and conversation history
            intent = app_manager.detect_application_intent(
                conversation_context=conversation_context,
                user_message=message
            )
            
            # Skip if intent is too casual
            if intent == ApplicationIntent.CASUAL_INQUIRY:
                return
            
            # Extract person name from conversation context or user info
            person_name = conversation_context.get("full_name") or user_id
            
            # Check if application should be created/linked
            ...
            if result[0]:  # Application found/created
                ...
                # Link conversation thread to application
                app_manager.link_conversation_to_application(thread_id, application_id)
                
                logger.info(f"Conversation {thread_id} linked to application {application_id} "
                          f"(status: {detection_status}, phase: {phase.value})")
                
        except Exception as e:
            logger.warning(f"Application lifecycle integration failed: {str(e)}")
            # Don't fail the chat if lifecycle integration fails
    
    def _extract_conversation_context(self, result: Dict[str, Any], context: MortgageContext) -> Dict[str, Any]:
        """Extract conversation context from chat result and runtime context"""
        
        conversation_context = {}
        
        # Add explicit context information
        if context.user_id:
            conversation_context["user_id"] = context.user_id
        if context.application_type:
            conversation_context["application_type"] = context.application_type
        if context.loan_officer_id:
            conversation_context["loan_officer_id"] = context.loan_officer_id
        ...
        if "messages" in result:
            ...
            # Simple extraction patterns for key information
            import re
            ...
            for pattern in name_patterns:
                match = re.search(pattern, messages_text, re.IGNORECASE)
                if match:
                    conversation_context["full_name"] = match.group(1).strip()
                    break
            ...
            for pattern in income_patterns:
                ...
                if match:
                    ...
                    try:
                        conversation_context["annual_income"] = int(income_str)
                    except ValueError:
                        pass
                    break
            
            # Estimate completion percentage based on detected fields
            detected_fields = len([k for k in conversation_context.keys() 
                                 if k in ["full_name", "annual_income", "employer", "purchase_price"]])
            conversation_context["completion_percentage"] = (detected_fields / 10) * 100  # 10 key fields
...
class MortgageConversationWorkflow:
    ...
    def chat(self, session_id: str, message: str) -> str:
        """
        Simple synchronous chat method for testing and basic usage.
        For production, use stream_chat or async methods.
        """
        try:
            # Use the agent's direct chat method
